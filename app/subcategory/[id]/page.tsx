...
import { fetchSubcategoryList } from "@/lib/api/stories"
...
export default async function SubcategoryListingPage({ params, searchParams }: Props) {
  ...
  if (isNaN(subcategoryId)) notFound()

  const items = await fetchSubcategoryList(subcategoryId, { limit, offset })
  const currentPage = Math.floor(offset / limit) + 1;
  const subcategoryName = subcategoryNames[subcategoryId] || `Subcategory ${id}`
      ...
      <main className="pt-6 pb-10">
        <div className="mx-auto max-w-7xl px-3 md:px-6">
          ...
          <div className="mt-12 flex items-center justify-center gap-2">
            {offset > 0 && (
              <Link
                href={`/subcategory/${id}?limit=${limit}&offset=${Math.max(0, offset - limit)}`}
            ...
            {items.length === limit && (
              <Link
                href={`/subcategory/${id}?limit=${limit}&offset=${offset + limit}`}
                className="px-4 py-2 rounded border border-gray-300 text-gray-700 hover:bg-gray-100 transition-colors"
              >
                Next →
              </Link>
