      ...
      <main className="pt-6 pb-10">
        <div className="mx-auto max-w-7xl px-3 md:px-6">
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((it) => (
              <li key={it.id} className="border rounded-lg overflow-hidden bg-white hover:shadow transition-shadow">
                <Link href={`/news/${it.id}`} className="block">
                  ...
                  <div className="p-4">
                    <span className="inline-flex items-center text-[10px] uppercase tracking-wide font-bold bg-yellow-400 text-black px-2 py-1 rounded mb-2">{it.category || params.slug}</span>
