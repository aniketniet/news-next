...
import { fetchTrendingNewsList } from "@/lib/api/stories"
...
export default async function TrendingPage({ searchParams }: Props) {
  ...
  const items = await fetchTrendingNewsList({ limit, offset })
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="pt-6 pb-10">
        <div className="mx-auto max-w-7xl px-3 md:px-6">
          ...
          {items.length === 0 && <p className="text-gray-600">No stories found.</p>}

          <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((it) => (
              <li
                key={it.id}
                className="border rounded-sm overflow-hidden bg-white hover:shadow transition-shadow"
              >
                <Link href={`/news/${it.urlKey || it.id}`} className="block">
                  <div className="relative aspect-[16/9] w-full bg-gray-100">
                    <Image
                      src={it.image_url_medium || "/news-image.jpg"}
                      alt={it.title}
                      fill
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-xs text-gray-500">
                      ...
                      {new Date(it.publishedDate).toLocaleDateString(undefined, {
                        day: "2-digit",
                        month: "short",
                        year: "numeric",
                      })}
          ...
          <div className="mt-12 flex items-center justify-center gap-2">
            {offset > 0 && (
              <Link
                href={`/trending?limit=${limit}&offset=${Math.max(0, offset - limit)}`}
            ...
            <span className="px-4 py-2 text-gray-700 font-medium">Page {currentPage}</span>

            {items.length === limit && (
              <Link
                href={`/trending?limit=${limit}&offset=${offset + limit}`}
                className="px-4 py-2 rounded border border-gray-300 text-gray-700 hover:bg-gray-100 transition-colors"
              >
                Next →
              </Link>
